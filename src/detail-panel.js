...
const els = {
  ...
  squawk: document.getElementById('detail-squawk'),
  badges: document.getElementById('detail-badges'),
  categoryCell: document.getElementById('detail-category-cell'),
...
export function initDetailPanel(onClose, onFollow) {
  ...
  btnIsolate.addEventListener('click', () => {
    ...
    btnIsolate.classList.toggle('active', isolateMode);
    btnIsolate.textContent = isolateMode ? 'SHOW ALL' : 'ISOLATE';
  });
...
export function isIsolating() {
  return isolateMode;
}

// ADS-B category codes to human-readable descriptions
const ADSB_CATEGORIES = {
  'A0': 'No info',
  'A1': 'Light (<15,500 lbs)',
...
const EMERGENCY_SQUAWKS = { '7500': 'HIJACK', '7600': 'COMMS FAIL', '7700': 'EMERGENCY' };

// Military sub-type classification by ICAO type designator (subset for display)
const MIL_SUBTYPES = new Map([
  ...['F16','F15','F15E','F18','FA18','F22','F35','F14','A10','EF2K','EUFI','RFAL','GRIP','JAS39','SU27','SU30','SU34','SU35','SU57','MG29','MG31','J10','J20'].map(t => [t, 'FIGHTER']),
  ...['AH64','H64','UH60','H60','CH47','H47','CH53','V22','AH1','AH1Z','NH90','MI24','MI28','KA52'].map(t => [t, 'HELO']),
  ...['B1','B1B','B2','B2A','B52','B52H','B21','TU95','TU160','H6'].map(t => [t, 'BOMBER']),
  ...['KC10','KC46','K135','KC35','KC30','MRTT'].map(t => [t, 'TANKER']),
  ...['U2','U2S','RC35','R135','E3','E3TF','E8','E2','E2C','E2D','E6','E6B','P3','P8','P8A','E4B','E7','RQ4','MQ9','MQ1'].map(t => [t, 'RECON/ISR']),
  ...['C17','C130','C5','C5M','A400','A40M','C295','IL76','AN124','Y20'].map(t => [t, 'TRANSPORT']),
  ...['T6','T38','T45','PC21','HAWK','M346','L39'].map(t => [t, 'TRAINER']),
]);

function getMilSubTypeLabel(ac) {
  const t = (ac.aircraft_type || '').toUpperCase();
  if (t && MIL_SUBTYPES.has(t)) return MIL_SUBTYPES.get(t);
  const cat = (ac.category || '').toUpperCase();
  if (cat === 'A7' || cat === 'B2') return 'HELO';
  if (cat === 'B4' || cat === 'B6') return 'UAV';
  return null;
}
...
export function updatePanel(aircraft) {
  ...
  if (aircraft.isGhost) {
    ...
    els.squawk.textContent = '░░░░';
    els.icao.textContent = '░░░░░░';
    els.badges.style.display = 'none';
    els.categoryCell.style.display = 'none';
    return;
  }
  ...
  // Squawk code with emergency highlighting
  const sq = aircraft.squawk || '';
  els.squawk.textContent = sq || '---';
  if (EMERGENCY_SQUAWKS[sq]) {
    els.squawk.textContent = `${sq} ${EMERGENCY_SQUAWKS[sq]}`;
    els.squawk.style.color = '#ff4444';
    els.squawk.style.fontWeight = '700';
  } else {
    els.squawk.style.color = '';
    els.squawk.style.fontWeight = '';
  }
  ...
  // ADS-B category
  if (aircraft.category && ADSB_CATEGORIES[aircraft.category.toUpperCase()]) {
    els.category.textContent = `${aircraft.category.toUpperCase()} - ${ADSB_CATEGORIES[aircraft.category.toUpperCase()]}`;
    els.categoryCell.style.display = '';
  } else {
    els.categoryCell.style.display = 'none';
  }

  // Badges: VIP, Military, Emergency
  const badges = [];
  const vip = getVipInfo(aircraft.icao24);
  if (vip) {
    badges.push(`<span class="detail-badge vip">${vip.owner} - ${vip.aircraft}</span>`);
  }
  const isMil = (aircraft.dbFlags & 1) !== 0;
  if (isMil) {
    const sub = getMilSubTypeLabel(aircraft);
    const label = sub ? `MILITARY [${sub}]` : 'MILITARY';
    badges.push(`<span class="detail-badge mil">${label}</span>`);
  }
  if (EMERGENCY_SQUAWKS[sq]) {
    badges.push(`<span class="detail-badge emer">SQUAWK ${sq} - ${EMERGENCY_SQUAWKS[sq]}</span>`);
  }
  if (badges.length > 0) {
    els.badges.innerHTML = badges.join('');
    els.badges.style.display = '';
  } else {
    els.badges.style.display = 'none';
  }
