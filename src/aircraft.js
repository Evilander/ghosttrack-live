...
// Points only — safe for clustered source (no mixed geometries)
export function toAircraftGeoJSON(aircraft) {
  const features = [];
...
// LineStrings only — velocity vectors, separate unclustered source
export function toVectorGeoJSON(aircraft) {
  const features = [];
...
// Legacy wrapper — kept for backward compat if anything still calls toGeoJSON
export function toGeoJSON(aircraft) {
  const points = toAircraftGeoJSON(aircraft);
  const vectors = toVectorGeoJSON(aircraft);
  points.features.push(...vectors.features);
  return points;
}
...
export async function fetchAircraft(map) {
  ...
  // Always fetch military and LADD globally regardless of zoom
  const specialPromises = [fetchSpecial('mil'), fetchSpecial('ladd')];
