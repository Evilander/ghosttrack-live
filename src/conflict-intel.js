...
// Casualty summary as of Mar 5, 18:00 GMT — AJLabs + CENTCOM + IRCS
const CASUALTY_DATA = {
  updatedAt: '2026-03-05T18:00:00Z',
  entries: [
    { region: 'Iran (total)', killed: '340+', wounded: '1,200+', status: 'reported', source: 'Iranian Red Crescent, IRNA' },
    { region: 'Iran — Minab school', killed: '148–158', wounded: '~95', status: 'partial', source: 'Iranian govt (footage verified WaPo/NYT)' },
    { region: 'Iran — infrastructure', killed: '~60', wounded: '~350', status: 'reported', source: 'IRCS, Al Jazeera field reports' },
    { region: 'Israel', killed: '14+', wounded: '~620', status: 'reported', source: 'MDA, hospitals, Magen David Adom' },
    { region: 'US Military', killed: '5', wounded: '12+ serious', status: 'confirmed', source: 'CENTCOM' },
    { region: 'UAE', killed: '3', wounded: '72', status: 'reported', source: 'AJLabs, UAE MOI' },
    { region: 'Kuwait', killed: '1', wounded: '38', status: 'reported', source: 'AJLabs' },
    { region: 'Iraq (PMF)', killed: '4', wounded: '11', status: 'reported', source: 'AJLabs, Iraqi MOD' },
    { region: 'Bahrain', killed: '0', wounded: '7', status: 'reported', source: 'AJLabs' },
    { region: 'Qatar', killed: '0', wounded: '19', status: 'reported', source: 'AJLabs' },
    { region: 'Oman', killed: '1', wounded: '8', status: 'reported', source: 'AJLabs, Oman Maritime' },
    { region: 'Cyprus (Akrotiri)', killed: '0', wounded: '2', status: 'reported', source: 'The Guardian, UK MOD' },
  ],
};
...
const DISPUTED_CLAIMS = [
  ...
  { claim: 'Trump: 9 Iranian naval ships sunk', status: 'disputed', source: 'NPR — CENTCOM confirmed 3 vessels sunk (corvette, 2 patrol boats)' },
  { claim: 'IRGC: 27 US bases attacked', status: 'claimed', source: 'IRGC via Al Jazeera' },
  { claim: 'IRGC: 560 US troops killed/injured', status: 'contradicted', source: 'CENTCOM confirms 5 KIA / 12 WIA' },
  ...
  { claim: 'Iran: nuclear sites undamaged', status: 'disputed', source: 'IAEA unable to verify. Satellite shows Natanz activity disrupted' },
  { claim: 'Houthi involvement in Hormuz mining', status: 'claimed', source: 'US 5th Fleet intercepts, not independently confirmed' },
];
...
export function initConflictIntel(map) {
  ...
  map.on('click', 'conflict-strikes-icons', (e) => {
    ...
    const html = `<div style="font-family:JetBrains Mono,monospace;max-width:360px;">` +
      `<div style="font-size:10px;letter-spacing:2px;color:${color};margin-bottom:5px;font-weight:700;">STRIKE LOCATION</div>` +
      `<div style="font-size:14px;font-weight:700;color:${color};text-shadow:0 0 8px rgba(255,50,50,0.3);">${esc(p.name)}</div>` +
      `<div style="font-size:12px;color:rgba(200,220,240,0.9);margin-top:6px;line-height:1.7;">${esc(p.desc)}</div>` +
      (p.casualties ? `<div style="font-size:12px;color:#ff4444;font-weight:700;margin-top:5px;">Casualties: ${esc(p.casualties)}</div>` : '') +
      `<div style="font-size:11px;margin-top:6px;">${verifiedBadge}</div>` +
      `<div style="font-size:10px;color:rgba(200,220,240,0.7);margin-top:4px;">Source: ${esc(p.source)}</div>` +
      `</div>`;
  ...
  map.on('click', 'conflict-airports-icons', (e) => {
    ...
    const html = `<div style="font-family:JetBrains Mono,monospace;max-width:360px;">` +
      `<div style="font-size:10px;letter-spacing:2px;color:${AIRPORT_COLOR};margin-bottom:5px;font-weight:700;">AFFECTED AIRPORT</div>` +
      `<div style="font-size:14px;font-weight:700;color:rgba(200,220,240,0.95);">${esc(p.name)} (${esc(p.icao)})</div>` +
      `<div style="font-size:13px;font-weight:700;color:${statusColor};margin-top:5px;">STATUS: ${esc(p.status)}</div>` +
      `<div style="font-size:12px;color:rgba(200,220,240,0.9);margin-top:5px;line-height:1.6;">${esc(p.reason)}</div>` +
      `</div>`;
  ...
  map.on('click', 'conflict-hormuz-fill', (e) => {
    const html = `<div style="font-family:JetBrains Mono,monospace;max-width:360px;">` +
      `<div style="font-size:10px;letter-spacing:2px;color:${HORMUZ_COLOR};margin-bottom:5px;font-weight:700;">MARITIME CHOKEPOINT</div>` +
      `<div style="font-size:14px;font-weight:700;color:${HORMUZ_COLOR};text-shadow:0 0 8px rgba(255,68,68,0.3);">STRAIT OF HORMUZ</div>` +
      `<div style="font-size:12px;color:rgba(200,220,240,0.9);margin-top:6px;line-height:1.7;">` +
      ...
      `<div style="font-size:11px;color:#ff8800;font-weight:700;margin-top:5px;">DE FACTO BLOCKADE - S&P Global/Platts</div>` +
      `</div>`;
...
async function fetchGdeltEvents() {
  try {
    ...
    const res = await fetch('/gdelt-conflict', { signal: controller.signal });
    clearTimeout(timeout);
    if (!res.ok) {
      updateGdeltFeed(); // clear loading indicator even on error
      return;
    }
    const geojson = await res.json();
    if (!geojson || !geojson.features) {
      updateGdeltFeed(); // clear loading indicator even on empty response
      return;
    }
    ...
    updateGdeltFeed(); // clear loading indicator even on exception
  }
}
...
function initBriefingPanel() {
  ...
  // Dynamic subtitle with day count
  const subtitle = document.querySelector('.conflict-panel-subtitle');
  if (subtitle) {
    const startDate = new Date('2026-02-28T00:00:00Z');
    const days = Math.floor((Date.now() - startDate.getTime()) / 86400000);
    subtitle.textContent = `Feb 28 -- Day ${days} -- 9+ countries affected`;
  }
...
function renderCasualtyTable() {
  ...
  const updatedDate = new Date(CASUALTY_DATA.updatedAt);
  const updatedStr = updatedDate.toISOString().replace('T', ' ').substring(0, 16) + ' UTC';
  ...
  container.innerHTML =
    ...
    `</div>` +
    `<div style="font-size:9px;color:rgba(224,240,255,0.4);margin-top:6px;">Last updated: ${updatedStr}</div>`;
}
...
function updateGdeltFeed() {
  ...
  if (gdeltEvents.length === 0) {
    container.innerHTML = '<div style="font-size:10px;color:rgba(224,240,255,0.4);padding:4px 0;">No GDELT events available for this region.</div>';
    return;
  }
