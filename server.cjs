...
function handleRequest(req, res) {
  ...
  if (req.url === '/gdelt-conflict') {
    ...
    // Primary: GEO API with simpler query (no quoted phrases which can break)
    const geoUrl = 'https://api.gdeltproject.org/api/v2/geo/geo?' +
      'query=Iran%20OR%20Israel%20OR%20Hormuz%20OR%20missile%20OR%20airstrike&' +
      'mode=PointData&format=GeoJSON&timespan=24h&maxpoints=75';

    // Fallback: DOC API → convert to GeoJSON
    const docUrl = 'https://api.gdeltproject.org/api/v2/doc/doc?' +
      'query=Iran%20Israel%20conflict%20OR%20airstrike%20OR%20missile&' +
      'mode=ArtList&format=json&timespan=24h&maxrecords=30&sort=DateDesc';

    function tryDocFallback() {
      const docReq = https.get(docUrl, (docRes) => {
        let body = '';
        docRes.on('data', (c) => { body += c; });
        docRes.on('end', () => {
          try {
            const data = JSON.parse(body);
            const articles = data.articles || [];
            const features = articles
              .filter(a => a.seendate && (a.socialimage || a.title))
              .slice(0, 40)
              .map((a, i) => {
                // DOC API doesn't always have coords, use a rough spread in the region
                const baseLat = 32 + (Math.random() - 0.5) * 10;
                const baseLon = 48 + (Math.random() - 0.5) * 15;
                return {
                  type: 'Feature',
                  geometry: { type: 'Point', coordinates: [baseLon, baseLat] },
                  properties: {
                    name: a.title || '',
                    html: a.url ? '<a href="' + a.url + '">link</a>' : '',
                    url: a.url || '',
                    domain: a.domain || '',
                    tone: a.tone ? parseFloat(String(a.tone).split(',')[0]) : 0,
                  },
                };
              });
    ...
    const gdeltReq = https.get(geoUrl, (gdeltRes) => {
      let body = '';
      gdeltRes.on('data', (c) => { body += c; });
      gdeltRes.on('end', () => {
        let hasFeatures = false;
        if (gdeltRes.statusCode === 200 && body) {
          try {
            const parsed = JSON.parse(body);
            hasFeatures = parsed.features && parsed.features.length > 0;
        ...
        if (hasFeatures) {
          ...
          // GEO API returned empty — try DOC API fallback
          tryDocFallback();
        }
      });
    }).on('error', () => {
